function test_Animate(s_mobility,s_input,time_step)
    ...
    for nodeIndex = 1:s_mobility.NB_NODES
        ...
        text_handles(nodeIndex) = text(vs_node(nodeIndex).v_x(1) + x_label_offset ,vs_node(nodeIndex).v_y(1),num2str(nodeIndex));
    end
    ...
    for timeIndex = 1:length(v_t);
        ...
        for nodeIndex = 1:s_mobility.NB_NODES
            ...
            set(text_handles(nodeIndex),'Position',[vs_node(nodeIndex).v_x(timeIndex) + x_label_offset,vs_node(nodeIndex).v_y(timeIndex),0])
        end
        drawnow;
    end
end